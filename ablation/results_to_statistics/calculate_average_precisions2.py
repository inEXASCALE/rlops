...
condition_ranges = [
    (1e1, 1e4, '10^1 to 10^4 (Low)'),
    (1e4, 1e7, '10^4 to 10^7 (Medium)'),
    (1e7, 1e9, '10^7 to 10^9 (High)')
]
...
base_threshold = 1e-7  # Tolerance from solver (tol=1e-7)
...
for lower, upper, label in condition_ranges:
    ...
    if not range_df.empty:
        ...
        # Compute average metrics for RL and fp64
        avg_metrics = {
            'Condition Range': label,
            'Num Matrices': len(range_df),
            'Success Rate': success_rate,
            'Avg RL Error': np.mean(range_df['rl_error'][np.isfinite(range_df['rl_error'])]),
            'Avg RL NBE': np.mean(range_df['rl_nbe'][np.isfinite(range_df['rl_nbe'])]),
            'Avg RL Rho': np.mean(range_df['rl_rho'][np.isfinite(range_df['rl_rho'])]),
            'Avg RL CBE': np.mean(range_df['rl_cbe'][np.isfinite(range_df['rl_cbe'])]),
            'Avg RL Iterations': np.mean(range_df['rl_iterations']),
            'Avg RL GMRES Iterations': np.mean(range_df['rl_gmres_iterations']),
            'Avg fp64 Error': np.mean(range_df['fp64_error'][np.isfinite(range_df['fp64_error'])]),
            'Avg fp64 NBE': np.mean(range_df['fp64_nbe'][np.isfinite(range_df['fp64_nbe'])]),
            'Avg fp64 Rho': np.mean(range_df['fp64_rho'][np.isfinite(range_df['fp64_rho'])]),
            'Avg fp64 CBE': np.mean(range_df['fp64_cbe'][np.isfinite(range_df['fp64_cbe'])]),
            'Avg fp64 Iterations': np.mean(range_df['fp64_iterations']),
            'Avg fp64 GMRES Iterations': np.mean(range_df['fp64_gmres_iterations'])
        }
