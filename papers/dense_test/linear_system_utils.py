...
def generate_random_linear_system(n=500, kappa=1000, random_state=42, estimate_condition=True):
    np.random.seed(random_state)
    random.seed(random_state)
    A = gallery_randsvd(n, kappa=kappa, mode=2, random_state=random_state)
    x_true = np.random.randn(n)
    b = A @ x_true
    cond_number = estimate_condition_number(A) if estimate_condition else float(kappa)
    metadata = {
        'condition_number': cond_number,
        'matrix_size': n,
        'kappa': kappa,
        'sparsity': compute_sparsity(A),
        'condition_estimated': bool(estimate_condition),
    }
    return A, b, x_true, metadata

def generate_datasets(num_train, num_test, size_range=(800, 1500), kappa_range=(3, 12), random_state=42, verbose=True, estimate_condition=True):
    np.random.seed(random_state)
    random.seed(random_state)
    train_set, test_set = [], []
    np.random.seed(random_state)
    ...
    for i in range(num_train + num_test):
        ...
        try:
            A, b, x_true, metadata = generate_random_linear_system(
                n=n, kappa=kappa, random_state=random_state + i, estimate_condition=estimate_condition
            )
...
class LinearSystemEnv:
    ...
    def compute_reward(self, x_rl, x_fp64, final_ferr, final_rho, final_nbe, final_cbe, precisions, gmres_iterations_sum, lu_failed, init_failed, fp64_rho, A_norm, b_norm):
        ...
        reward = accuracy_reward + precision_reward  - iteration_penalty
