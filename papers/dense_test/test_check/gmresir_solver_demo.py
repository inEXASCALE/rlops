...
def load_data(num_train, num_test, data_dir='data_random_dense5'):
    train_data = []
    test_data = []
    train_metrics = []
    test_metrics = []
...
def check_data_files(num_train, num_test, data_dir='data_random_dense5'):
...
def save_data(train_data, test_data, data_dir='data_random_dense5'):
    os.makedirs(data_dir, exist_ok=True)
...
class GMRESIR5Solver:
    ...
    def ir5_gmres_mixed_precision(self, A, b, x_true, matrix_id, phase="test", precisions=None):
        ...
        # 数据准备
        A_torch = A.to(dtype=torch.float64, device=self.device).coalesce()
        ...
        b_norm = torch.norm(b_torch).item()

        if precisions is None:
            # RL action logic...
            uf, ug, u, ur = 'fp16', 'fp16', 'fp32', 'fp32' # default
        else:
            uf, ug, u, ur = precisions # ug = Uniform GMRES precision

        precision_log = [[uf, ug, u, ur]]
        errors = []
        total_gmres_iterations = []
        
        # 1. 准备 Working Precision 数据
        sim_u = FastPrecisionSimulator(u, device=self.device)
        if u == 'fp64':
            A_torch_rounded = A_torch
            b_torch_rounded = b_torch
        else:
            # 稀疏矩阵值截断
            val_rounded = sim_u.q(A_torch.values())
            A_torch_rounded = torch.sparse_coo_tensor(A_torch.indices(), val_rounded, A_torch.shape).coalesce()
            b_torch_rounded = sim_u.q(b_torch)

        # 转 Scipy 给 LU
        vals = A_torch_rounded.values().cpu().numpy()
        rows = A_torch_rounded.indices()[0].cpu().numpy()
        cols = A_torch_rounded.indices()[1].cpu().numpy()
        A_scipy = coo_matrix((vals, (rows, cols)), shape=A.shape)
        
        # 2. LU 分解 (uf)
        lu, lu_failed = self.compute_lu_factorization(A_scipy, uf, phase)
        if lu is None: return self._failure_return(size)

        # 3. 初始解 (Precond solve at fp64 or uf inside LU)
        x, init_failed = self.initialize_solution(lu, b_torch, phase)
        if x is None: x = torch.zeros(size, 1, dtype=torch.float64, device=self.device)

        # 4. 初始残差 (ur)
        sim_ur = FastPrecisionSimulator(ur, device=self.device)
        r = b_torch_rounded - torch.sparse.mm(A_torch_rounded, x)
        if ur != 'fp64': r = sim_ur.q(r)
        
        # Initial Metrics
        self._record_metrics(errors, x, x_true, r, A_scipy, b_torch, 0)
        
        total_iterations = 0
        converged = False
        prev_dx = float('inf')

        # --- Uniform GMRES Simulator Setup ---
        # 专门用于 GMRES 内部算术的模拟器 (ug)
        ug_simulator = FastPrecisionSimulator(ug, device=self.device)

        # 定义 A_func 和 M_func 供 torch_uniform_gmres 使用
        def A_func_gmres(v):
            return torch.sparse.mm(A_torch_rounded, v)

        def M_func_gmres(v):
            res, _ = self.apply_preconditioner_torch(lu, v, ug, phase)
            return res

        min_nbe = errors[-1][0] # 初始 NBE
        ...
        while total_iterations < self.max_iter:
            ...
            # 5. 调用 GPU 加速的 Uniform GMRES
            # 输入 r 已经在 GPU 上
            d, gmres_info = torch_uniform_gmres(
                A_func_gmres, r, 
                maxiter=self.gmres_max_it, tol=self.tol, 
                M_func=M_func_gmres, simulator=ug_simulator
            )
            
            converged_gmres = (gmres_info == 0)
            total_gmres_iterations.append(self.gmres_max_it if gmres_info != 0 else 0) # 简化计数
            
            # 6. 更新量截断 (u)
            if u != 'fp64': d = sim_u.q(d)
            
            # 7. 收敛检查
            dx_norm = torch.norm(d, float('inf')).item()
            x_norm = torch.norm(x, float('inf')).item()
            if x_norm > 0: rel_dx = dx_norm / x_norm
            else: rel_dx = float('inf')
            
            if rel_dx <= self.fp64_unit_roundoff:
                converged = True
                break
            
            current_precision_eps = self.unit_roundoffs.get(u, 1e-5) 
            
            if rel_dx < current_precision_eps:
                 logging.info(f"Stagnation detected: Relative update {rel_dx:.2e} is below working precision limit ({current_precision_eps:.2e}). Stopping.")
                 stagnation = True
                 break


            # 或者简单的滑动窗口检测：如果 Error 连续 3 次没变
            if len(errors) > 0:
                current_nbe = errors[-1][0] # 获取当前的 NBE
                
                # 如果当前误差比历史最小值还小（有明显改进）
                # 这里加了一个极小的 epsilon (1e-12) 防止浮点数相等时的抖动，也可以直接用 <
                ...
                if stagnation_counter >= self.max_stagnation:
                    logging.info(f"Stagnation detected: NBE did not improve for 3 iterations (Min: {min_nbe:.2e}, Curr: {current_nbe:.2e}). Stopping.")
                    stagnation = True
                    break
            ...
            # 9. 更新残差 (ur)
            ax = torch.sparse.mm(A_torch_rounded, x)
            if ur != 'fp64': ax = sim_ur.q(ax)
            r = b_torch_rounded - ax
            if ur != 'fp64': r = sim_ur.q(r)
            
            self._record_metrics(errors, x, x_true, r, A_scipy, b_torch, total_iterations)
            
        return x, total_iterations, total_gmres_iterations, errors, precision_log, 0, 0, 0, 0, converged, stagnation, converged_gmres, False, False, False
    ...
    def ir5_gmres_fp64(self, A, b, x_true, matrix_id, phase="test"):
        ...
        lu, _ = self.compute_lu_factorization(A_scipy, 'fp64', phase="test")
        ...
        return x, total_iterations, total_gmres_iterations, errors, final_nbe, final_ferr, final_rho, final_cbe

    def train(self, train_data, top_k=20, episodes=100, results_dir='results_random_dense5'):
    ...
    def _failure_return(self, size):
        return torch.zeros(size, 1, device=self.device), self.max_iter, [], [], [], 0,0,0,0, False, False, False, True, False, False
    ...
    def test(self, test_data):
        ...
        for i, (A_scipy, b_np, x_true_np, metadata) in enumerate(tqdm(test_data, desc="Testing")):
            ...
            x_fp64, iters_fp64, iters_fp64_gmres, errors_fp64, final_nbe_fp64, final_ferr_fp64, final_rho_fp64, final_cbe_fp64 = self.ir5_gmres_fp64(A_scipy, b, x_true, matrix_id=i)
            ...
            for p in precisions:
                precision_usage[p] += 1
            x_rl, iters_rl, iters_rl_gmres, errors_rl, log_rl, final_nbe_rl, final_ferr_rl, final_rho_rl, final_cbe_rl, _, _, _, _, _, _ = \
...
if __name__ == "__main__":
    ...
    tol = 1e-9
    episodes = 1
    data_dir = 'data_random_dense5'
    results_dir = 'results_random_dense5'

    train_data, test_data, train_metrics, test_metrics = load_data(num_train, num_test, data_dir)
    if train_data is None or test_data is None:
        logging.info("Generating new datasets...")
        train_data, test_data = generate_datasets(num_train, num_test, size_range=(100, 300), kappa_range=(1, 9), verbose=True)
