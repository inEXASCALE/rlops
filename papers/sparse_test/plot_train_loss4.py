...
def plot_training_progress(results_dir='results_random_unsym'):
    ...
    plt.grid(True)
    plt.savefig(os.path.join(results_dir, 'episode_rewards_plot_sparse4.png'))
    plt.close()
    print(f"Saved episode rewards plot to {os.path.join(results_dir, 'episode_rewards_plot_sparse4.png')}")
    ...
    if os.path.exists(losses_path):
        ...
        plt.tick_params(axis='both', which='major', labelsize=15)
        plt.grid(True)
        plt.savefig(os.path.join(results_dir, 'training_losses_plot_sparse4.png'))
        plt.close()
        print(f"Saved training losses plot to {os.path.join(results_dir, 'training_losses_plot_sparse4.png')}")
    else:
...
if __name__ == "__main__":
    results_dir = 'results_random_sparse4'
